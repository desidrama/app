// HomeScreen: Main home screen with carousel, trending videos, and continue watching.
...
// Type for carousel banner items
...
/**
 * CarouselVideoPlayer: Plays a video preview in the carousel if active.
 */
...
/**
 * HomeScreen: Main entry for the home tab, shows carousel, trending, and continue watching.
 */
export default function HomeScreen() {
  ...
  // State and refs for carousel, trending, and video playback
  ...
  /**
   * fetchVideoUrlForCarouselItem: Tries to find a video URL for a carousel item.
   */
  ...
  /**
   * refreshHomeContent: Loads latest, trending, and popular video data for home screen.
   */
  ...
  // Pull-to-refresh hook for home content
  const {
    refreshing,
    onRefresh,
  ...
  // Scroll carousel to initial position after loading
  useEffect(() => {
  ...
  // Auto-play video preview in carousel
  useEffect(() => {
  ...
  /**
   * handleMainScroll: Handles main scroll and stops video preview if needed.
   */
  ...
  // Stop video preview when screen loses focus
  useFocusEffect(
    useCallback(() => {
      return () => {
        setActiveVideoIndex(null);
  ...
  /**
   * onCarouselScrollBegin: Stops video preview when carousel scroll starts.
   */
  ...
  /**
   * fetchContinueWatching: Loads continue watching data from API.
   */
  ...
  // Animated event for carousel scroll
  ...
  /**
   * onCarouselScrollEnd: Handles infinite carousel looping logic.
   */
  ...
  /**
   * handleCarouselPress: Handles press on carousel item and navigates to content.
   */
  const handleCarouselPress = async (item: CarouselBannerItem, displayIndex: number) => {
    try {
      // Always use the item passed in, not recalculated index
      ...
      // Fallback: videoUrl is not a valid param, so just navigate to Reels
  ...
  /**
   * handleContinueWatchingPress: Navigates to video from continue watching.
   */
  ...
  /**
   * handleVideoPress: Navigates to video details from trending list.
   */
  ...
  /**
   * handleMuteToggle: Toggles mute state for video preview.
   */
  ...
  const dynamicStyles = StyleSheet.create({
    ...
    heroCard: {
      ...
      shadowOpacity: 0,
      borderWidth: 2,
      borderColor: theme === 'dark' ? colors.surface : '#000000',
    },
    heroCardActive: {
  ...
  return (
    ...
    >
      ...
      <SafeAreaView style={dynamicStyles.safeAreaInner}>
                  ...
                  <Animated.FlatList
                    ...
                    renderItem={({ item, index }) => {
                      ...
                      const getContentTypeLabel = () => {
                        if (item.contentType === 'webseries') {
                          return item.episodeCount ? `${item.episodeCount} eps` : '';
                        } else if (item.contentType === 'reels') {
                        ...
                        >
                          <Animated.View
                            style={[
                              ...
                              isVideoActive && dynamicStyles.heroCardActive,
                            ]}
                          >
                            <Animated.View 
                              style={[
                ...
                >
                  {latestTrendingData.map((item, i) => (
                    <VideoCard
                      ...
                      seriesName={item.seasonId?.title}
